import { a as e, f as t, i as n, n as r, t as i } from "./util-oQvU84AW.js";
import a from "node:fs";
import o from "node:path";
import s from "node:fs/promises";
//#region src/project/exports.ts
var c = (e, n, i = !1) => {
	if (!n.exports) return { val: {
		dist: "dist",
		src: "src",
		exports: []
	} };
	if (typeof n.exports == "string") return { err: "'exports' must be the object form in order for types to be respected" };
	if (n.types) return { err: "'types' field must not be specified in package.json; use exports.<entry>.types" };
	let s = n.exports, c = "", l = "", u = [];
	for (let n in s) {
		let d = s[n];
		if (typeof d == "string") {
		...
		if (n.includes(" ")) return { err: `entry name must not contain space: '${n}'` };
		if (n === "index") return { err: "entry name must not be \"index\", use \".\" instead" };
		if (n === "_dts_") return { err: `entry name must not be "${t}"` };
		if (n !== ".") {
			if (!n.startsWith("./")) return { err: "entry name subpath must start with './'" };
			if (n = n.substring(2), n.includes("/")) return { err: "entry name cannot contain '/' other than the initial './'" };
			if (n.includes(".")) return { err: "entry name cannot contain '.' other than the initial './'" };
		}
		let f = d.import, p = d.types;
		if (!f || !p) return { err: "exports value must be object with \"import\" and \"types\"" };
		if (!f.startsWith("./")) return { err: `import path must start with './' (for entry point '${n}')` };
		...
		if (!f.endsWith(".js")) return { err: `import path must end with ".js": ${f} (for entry point '${n}')` };
		...
		if (p = p.substring(c.length + t.length + 4), l) {
			...
			let [e, n] = r(p, "/");
			if (!n) return { err: `types path must be in the format of "./${c}/${t}/<src>/<file>.d.ts"` };
			l = e.trim(), p = n;
		}
		...
		i && console.log(`[mono] configured entry "${n}": ${l}/${f}.ts${h ? "x" : ""}`), u.push({
			entry_name: n,
			source_path_abs: m,
			dist_path_rel: f + ".js"
		});
	...
	if (u !== -1) {
		let e = f ? "" : ",";
		t ? l.splice(u, d - u + 1, `    "imports": ${o}${e}`) : l.splice(u, d - u + 1), p = i(l.join("\n"));
	} else if (t) p = i((c.endsWith("}") ? c.substring(0, c.length - 1) : c).trimEnd() + `,\n    "imports": ${o}\n}`);
	else return console.log("[mono] subpath import mapping is up-to-date"), {};
	let m = { ...r };
	t ? m.imports = t : delete m.imports;
	let h = i(n(m) || "");
	try {
		if (h !== i(n(JSON.parse(p)) || "")) return { err: "failed to edit 'imports' in package.json: failed to edit 'imports'. Please delete the field manually and retry" };
	} catch {
	...
	return await s.writeFile(a, p), t ? r.imports = t : delete r.imports, console.log("[mono] updated subpath import mapping"), {};
};
//#endregion
export { l as n, c as r, u as t };

//# sourceMappingURL=project-Hf_DyJ4s.js.map