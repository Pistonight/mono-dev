...
export default function monodev(monoConfig) {
    return (config) => {
        ...
        if (config.build.chunkSizeWarningLimit) {
            console.warn("[mono-dev] not injecting chunk size warning limit because it is already specified")
        } else {
        ...
        if (!config.build.manualChunks) {
            ...
            for (const key in ManualChunks) {
                if (config.build.manualChunks[key]) {
                    console.warn(`[mono-dev] not injecting manual chunk ${key} because it is already specified`)
                } else {
