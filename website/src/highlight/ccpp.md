...
static void handle_mapping(
    ...
    do {
        // Query next range
        if (/* @tree-sitter:function.macro */R_FAILED(svcQueryMemory(&meminfo, &pageinfo, meminfo.addr + meminfo.size))) {
            /* @tree-sitter:function.macro */panic_("mirror: svcQueryMemory failed");
        }
        ...
        if (map) {
            if (/* @tree-sitter:function.macro */R_FAILED(svcMapProcessMemory(rw_start, process, ro_start, size))) {
                /* @tree-sitter:function.macro */panic_("mirror: svcMapProcessMemory failed");
            }
        } else {
            if (/* @tree-sitter:function.macro */R_FAILED(svcUnmapProcessMemory(rw_start, process, ro_start, size))) {
                /* @tree-sitter:function.macro */panic_("mirror: svcUnmapProcessMemory failed");
            }
        }
...
Mirror::Mirror(uintptr_t start, size_t size) {
    ...
    /* @tree-sitter:function.macro */assert_(rw_start_aligned != 0);
    ...
    /* @tree-sitter:function.macro */assert_(reserve != nullptr);
    ...
    // NOLINTNEXTLINE(performance-no-int-to-ptr) FIXME
    /* @tree-sitter:function.macro */assert_(memcmp(reinterpret_cast<void*>(m.ro_start), reinterpret_cast<void*>(m.rw_start),
                   size) == 0);
}
